...
# thijs hosman, Aug 20 2015
# TODO: some kind hierarchy in event listener/handler so that not every object gets all events
# TODO: add Twisted for network communication
...
class EventManager(object):
	...
	communicates between the Model, View, and Controller.'''

	# all events are sent to all listeners
	# todo: send event to listeners in the right order: when a tick is created:
	# 1. get the controllers polled
	# 2. send events they generate to the model and view
	# 3. make sure the events they generate are forwarded
	# 4. tick the model
	# 5. forward the events they generate
	# 6. tick the view so that the screen updates

	# probably need 4 types of events (tick-controller, tick-model, tick-view and non tick)

	# NB: sends all events to all listeners, and all modules (view, model and controller) are registered.
	# this is obviously not the nicest way of handling this, but the hierarchy that is needed otherwise is a lot of work to code. 

	def __init__(self):
		# use weakref so that if the ref dies the entry will be removed
...
class KeyboardController(object):
	'''a simple keyboardcontroller. KeyboardController takes Pygame events generated by the
...
class CPUSpinnerController(object):
	"""generate tick events that tell the View to keep updating"""
	def __init__(self, evManager):
		self.evManager = evManager
		self.evManager.RegisterListener( self )
		self.keepGoing = 1
...
class PygameView(object):
	...
	def __init__(self, evManager):
		...
		pygame.display.set_caption( 'Wingman Demo' )
...
class Charactor(object):
	"""MODEL: this is the character the player controls"""
...
def main():
	...
	# initialze keyboard controller and register it with the event manager
	...
	# let the spinner start so that the view starts updating
	spinner.Run()
